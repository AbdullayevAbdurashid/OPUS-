...
function Search(props) {
    ...
    // }
    const [block, setBlock] = React.useState({ display: "none" });
    ...
    return (
        <div className="searcher" style={{ width: 400 }}>
            <div {...getRootProps()}>
                ...
                <input onKeyDown={(e) => { e.target.value.length >= 0 ? setBlock({ display: "block" }) : setBlock({ display: "none" }) }} placeholder='Type here to search 🔎' className='inputs' {...getInputProps()} />
            </div>
            {groupedOptions.length > 0 ? (
                <ul id='listbox' style={block} className='listbox' {...getListboxProps()}>
